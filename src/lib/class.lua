--[[
    Titanium Class System - Version 1.2 (Repurposed for Discord bot [discord-organiser])

    Copyright (c) Harry Felton 2016 - 2017

    **************************************************************************************
    ** This class system has been repurposed for a Discord bot. The bot, being written  **
    **     in Lua can make use of this class system to help enforce OOP ideologies.     **
    **                                                                                  **
    ** This code, retains its original license (MIT) -- as provided by the bots license **
    **   The Titanium license is identical and can thus be ignored in most instances.   **
    **************************************************************************************
    ...
    More on (*)
    ===========
    Instead of classes being automatically exported into _G, they are now simply made available
    to the file. It is the files responsibility to compile AND return the class itself out of
    the file.

    When the file is called upon via `require`, the class will be returned automatically (thanks
    to requires built in cache) without re-compilation.
]]
...
--[[ Constants ]]--
local ERROR_BUG = "\nPlease report this via GitHub @ hbomb79/discord-organiser"
            ...
            for key, value in pairs( tbl.static ) do
                if reserved[ key ] then
                    throw(
                        ...
                        "'"..key.."' is reserved for internal processes."
                    )
                end
...
local function spawn( target, ... )
    ...
    if classReg.abstract or not classReg.compiled then
        print"WARNING: CANNOT instantiate"
        -- throw(
...
function class( name )
    ...
    elseif reserved[ name ] then
        nameErr ("'"..name.."' is reserved for internal processes")
    else
    ...
    classObj.compile = function() compileCurrent(); return classObj; end
...
function abstract( eol )
    ...
    return eol and currentClass or propertyCatch
end
...
-- print( "Class: " .. tostring( require "src.lib.class" ) )
-- print( "abstract: " .. tostring( abstract ) )

-- print( "discordia: " .. tostring( require "discordia" ) )
